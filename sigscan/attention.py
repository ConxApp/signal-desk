...
def rolling_z(series: Sequence[float], baseline_days: int = 45, min_history: int = 14) -> list:
...
def acceleration(series: Sequence[float], baseline: Sequence[float]) -> float:
    """Last 3 days vs the 4 before, in robust-sigma units of the baseline."""
    if len(series) < 8:
        return 0.0
...
def trend_stage(z_series: Sequence[float], accel: float) -> str:
    """quiet | rising | peaking | fading, judged over the last 14 days."""
    if not z_series:
        return "quiet"
...
class Attention:
    ...
    z_spark: list = field(default_factory=list)     # driver channel z, last 30 days
    driver_spark: list = field(default_factory=list)  # driver channel raw values, last 30 days
...
def score_attention(history: Sequence[DailyObservation], baseline_days: int = 45,
    ...
    """Score the most recent day of one entity's history across every channel."""
    if not history:
        return None
    today = history[-1]
    n = len(history)
    dates = [o.date for o in history]

    # --- channel series ---------------------------------------------------
    wiki = [math.log1p(o.wiki_views) for o in history]
    news = [math.log1p(o.news_articles) for o in history]
    app = [app_signal(o.app_rank) for o in history]
    has_social = any(o.consumer_denominator > 0 for o in history)
    social = [log_share(o.consumer_mentions, o.consumer_denominator) for o in history] if has_social else []

    has_wiki = any(o.wiki_views > 0 for o in history)
    has_news = any(o.news_articles > 0 for o in history)
    has_app = any(o.app_rank > 0 for o in history)

    zs = {}
    series_by = {}
    if has_wiki:
        zs["wiki"] = rolling_z(wiki, baseline_days); series_by["wiki"] = wiki
    if has_news:
        zs["news"] = rolling_z(news, baseline_days); series_by["news"] = news
    if has_app:
        zs["app"] = rolling_z(app, baseline_days); series_by["app"] = app
    if has_social:
        zs["social"] = rolling_z(social, baseline_days); series_by["social"] = social
    ...
    channel_z = {}
    for ch, series in zs.items():
        z = series[-1]
        if ch == "social":
            z = z * gate if z > 0 else z
        if ch == "news":
            z = z * 0.9          # news is noisier and often a lagging echo
    ...
    attention_z = base_z + min(0.4 * len(corroborating), 1.2)
    attention_z = round(min(attention_z, 12.0), 2)

    drv_series = series_by[driver]
    drv_z = zs[driver]
    ...
    spikes = spike_dates(dates, drv_z)
    fwd = forward_returns(dates, [o.close for o in history], spikes, horizon=10) if has_prices else []
    past = {"n": len(fwd),
    ...
    if has_app and channel_z.get("app", 0) >= 2.0:
        flags.append("APP_CLIMBING")
        why.append(f"app is climbing the App Store chart (rank {int(today.app_rank)})")
    if finance_noticed:
    ...
    last30 = history[-30:]
    price_spark = [o.close for o in history if o.close > 0][-30:]
    return Attention(
        ...
        acceleration=round(accel, 2), trends_hit=int(today.trends_hit),
        ...
        price_spark=[round(c, 4) for c in price_spark],
    )
